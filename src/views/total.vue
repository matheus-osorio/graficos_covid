...
export default {
  data(){
    return {
      ...
      total:{
        title: 'TOTAL VISITAS',
        subtitle: 'Total de Visitas Até Hoje',
        text: 0
      },
      mensal:{
        title: 'VISITAS DO MÊS',
        subtitle: 'Total de Visitas desse Mês',
        text: 0
      },
      hoje:{
        title: 'VISITAS HOJE',
        subtitle: 'Total de Visitas Hoje',
        text: 0
      },
    }
  },
  ...
  methods:{
    ...
    trocaMensal(num){
      ...
      this.graficos.valor = this.api[this.atual].total
      this.graficos.data = this.api[this.atual].data

      this.mensal.text = this.api[this.atual].total.reduce((soma,atual) => soma + atual)
      this.data = this.api[this.atual].data
      if(this.atual == this.maximo){
        const hoje = (new Date()).getDate()
        this.hoje.text = this.api[this.atual].total[hoje]
      }
      else{
  ...
  async mounted(){
    ...
    const total = api.reduce((valor, arr) => {
      valor += arr.total.reduce((soma,total) => soma + total)
      return valor
    },0)
